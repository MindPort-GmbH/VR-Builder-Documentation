...
- **Objects**
    ...
    The `Process Scene Objects` which should be inside the collider.
...
- An `Object` counts as inside the collider if its transform position is inside the `Collider`. This means a 3D object can count as inside even if it is only partially in the `Collider`. You can have one grabbable object with multiple child `Objects` (for example, four on the cube corners) to ensure that the whole object is inside.
